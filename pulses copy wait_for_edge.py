...
the 10sec pulse occurs the ">5secs" allows immediate update. can be used in
conjunction with emonhub's reporter (send) interval if a short interval is
preferred so that more timestamps are recorded eg update every 1sec to emonhub
...
def processpulse(channel,status):
    ...
    t = time.time()
    f = ' '.join((str(t), str(nodeid), str(pulse_id[1])))
